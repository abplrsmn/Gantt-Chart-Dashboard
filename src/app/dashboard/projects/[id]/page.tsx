...
type WeekEntry = {
  week: number;
  monthKey: string;   // "Apr 2025" — used for grouping
  monthLabel: string; // "April 2025" — shown in tab
  range: string;
  plan: number;
  actual: number;
  status: string;
...
function WeeklyProgressSection({ startDate, endDate }: { startDate: string | null; endDate: string | null }) {
  const weeks  = buildWeeks(startDate, endDate);
  const months = Array.from(new Map(weeks.map(w => [w.monthKey, w.monthLabel])).entries());
  const [activeKey, setActiveKey] = useState<string>(months[months.length - 1]?.[0] ?? "");
...
// ─── WeekCard ─────────────────────────────────────────────────────────────────
function WeekCard({ week, range, plan, actual, status, photos }: {
  week: number;
  range: string;
  plan: number;
  actual: number;
  ...
  return (
    <div className="rounded-xl border border-slate-200/70 dark:border-white/8 overflow-hidden">
      ...
      <div className="px-4 py-2.5 flex items-center gap-3" style={{ backgroundColor: "rgba(251,191,36,0.14)" }}>
        ...
        <span className="text-[10px] font-semibold text-amber-600/80 dark:text-amber-500/80">
          {range}
        </span>
        ...
        {photos === 0 ? (
          ...
          <div className={`grid gap-2 ${colsClass}`}>
            {Array.from({ length: Math.min(photos, 6) }).map((_, i) => (
              <div
                key={i}
                className="aspect-video rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200/60 dark:border-white/8 flex items-center justify-center"
              >
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-50 dark:bg-white/3 rounded-lg px-3 py-2.5">
            ...
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">
              {plan > 0 ? `${plan.toFixed(2)}%` : "—"}
            </p>
          </div>
          <div className="bg-slate-50 dark:bg-white/3 rounded-lg px-3 py-2.5">
            <p className="text-[9px] uppercase tracking-widest text-slate-400 mb-1">Actual Progress</p>
            <p className={`text-sm font-bold ${
              actual <= 0      ? "text-slate-400 dark:text-slate-600"
              : actual >= plan ? "text-emerald-600 dark:text-emerald-400"
          ...
          <div className="bg-slate-50 dark:bg-white/3 rounded-lg px-3 py-2.5">
            ...
            <p className={`text-sm font-bold ${
              ...
              : variance < 0 ? "text-rose-500 dark:text-rose-400"
...
export default function ProjectDetailPage() {
  ...
  return (
    <div className="space-y-4 pb-10 animate-page-enter">
      ...
      {/* ── Weekly Progress ──────────────────────────────────────────────── */}
      <WeeklyProgressSection
        startDate={project.pm_start ?? project.start_date}
        endDate={project.pm_end ?? project.end_date}
      />
